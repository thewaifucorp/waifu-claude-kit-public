...
- **Logging → structlog, configured once. NEVER `print()`.** Not for diagnostics, not for "structured stdout events" — those go through the logger too (`log.info("savings", **event)`, never `print(json.dumps(event))`). **Mandatory redaction processor** masks keys containing secret/token/password/dsn/api_key/cpf/email → `***`; **never log a secret/PII value.** Console in dev, JSON in prod (by setting). Don't re-author the processor per repo — import the shared one (`katsui-logging`); copy-pasting is how a repo forgets it.
